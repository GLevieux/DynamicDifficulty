...
load_data <- function(DTGame, name, sexe, age){
  ...
  for( i in 2:nrow(DT)){
    ...
    DT[i]$variation.Model = sqrt(delta/length(seq(0.1,0.9,0.05)))
    variationTotale = variationTotale + sqrt(delta/length(seq(0.1,0.9,0.05)))
  }
...
plot(x=DTGame$step,y=DTGame$variation.Model, pch = 20, col=rgb(0.1, 0.2, 0.8, 0.2))
...
for(i in 1:nrow(DTLevel)){
  ...
  DT = data.table(difficulty=diffs,
                  ...
                  estDiff = predicted)
  if(i==1){
    p = ggplot(DT,aes(x=difficulty,y=estDiff)) + geom_path(alpha = 0.2,colour=rgb(DT$variation*15, 0.0, 0.0, 1.0-DT$variation*15)) + xlim(0,1) + ylim(0,1)
  } else {
    p = p + geom_path(x=DT$difficulty,y=DT$estDiff, alpha = 0.2,colour=rgb(DT$variation*15, 0.0, 0.0, 1.0-DT$variation*15))
  }
}
