...
load_data <- function(DTGame, name, sexe, age){
  ...
  DT$variation.Model.Total = variationTotale / nrow(DT)
...
for(i in 1:nrow(DTLevel)){
  ...
  DT = data.table(difficulty=diffs,
                  ...
                  estDiff = predicted)
  if(i==1){
    p = ggplot(DT,aes(x=difficulty,y=estDiff)) + geom_path(alpha = 0.2,colour=rgb(DT$variation*15, 0.0, 0.0, 1.0)) + xlim(0,1) + ylim(0,1)
  } else {
    p = p + geom_path(x=DT$difficulty,y=DT$estDiff, alpha = 0.2,colour=rgb(DT$variation*15, 0.0, 0.0, 1.0))
  }
}
