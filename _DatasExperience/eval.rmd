...
DTVarModel = DTGame[used.Model==1,.(mean(variation.Model),mean(param.Diff)), by = idPlayer]
#barplot(DTVarModel$V1,names.arg=DTVarModel$idPlayer)
hist(DTVarModel$V1,main = "Histogram of model variability")
hist(DTVarModel$V2,main = "Histogram of player levels")
...
For each player, we computed the mean variation of model predictions when the model was used : `r kable(DTVarModel[order(V1)])`
